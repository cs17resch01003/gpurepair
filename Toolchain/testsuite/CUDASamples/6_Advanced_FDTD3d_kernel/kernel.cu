//pass
//--gridDim=[12,24]      --blockDim=[32,16] 2
                                        ...
                                        const int dimz)
{
    ...
    for (int i = RADIUS - 2 ;
         __invariant((inputIndex - ((RADIUS + gtidy) * stride_y + RADIUS + gtidx)) == stride_z * ((RADIUS - 2) - i)),
         __invariant(i >= -1),
         i >= 0 ; i--)
    {
        if (validr)
